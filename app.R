                              ...
                              fluidRow(
                                ...
                                column(5,
                                       conditionalPanel("output.status_app_output == 'No Baseline' | output.status_app_output == 'Ok Baseline'",
                                                        br(), 
                                                        div(class = "baseline_left",
                                                            bsButton("open_country_param", label = "Province", icon = icon('cog'), style = "primary", type = "action", value = FALSE, 
                                                                     block = TRUE), br(),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$province_cases, if(input$province_cases != "-- Own Value ---"){
    cases_rv$data <- casesprv %>% filter(province == input$province_cases) %>%
      select(-province)

    prv <- THprv_frac %>% filter(province==input$province_cases)
    population_rv$data$pop <- sum(THpopstruc) * prv$frac * THpopstruc_frac
